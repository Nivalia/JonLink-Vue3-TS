...
import type { GenQuejlParams, GenTable, GenTableInfoResult, AjaxResult, TableDataInfo } from '@/types'

// 查询生成表数据
export function listTable(query: GenQuejlParams): Promise<TableDataInfo<GenTable>> {
  return request({
    url: '/tool/gen/list',
    method: 'get',
    params: query
...
// 查询db数据库列表
export function listDbTable(query: GenQuejlParams): Promise<TableDataInfo<GenTable>> {
  return request({
