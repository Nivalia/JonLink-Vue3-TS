...
import type { RoleQuejlParams, AuthUserQuejlParams, SysRole, SysUser, SysUserRole, AuthUserSelectParams, AjaxResult, RoleDeptTreeResult, TableDataInfo } from '@/types'

// 查询角色列表
export function listRole(query: RoleQuejlParams): Promise<TableDataInfo<SysRole[]>> {
  return request({
...
// 查询角色已授权用户列表
export function allocatedUserList(query: AuthUserQuejlParams): Promise<TableDataInfo<SysUser[]>> {
  return request({
...
// 查询角色未授权用户列表
export function unallocatedUserList(query: AuthUserQuejlParams): Promise<TableDataInfo<SysUser[]>> {
  return request({
