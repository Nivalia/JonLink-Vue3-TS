<template>
  <div class="app-container">
    <el-form :model="quejlParams" ref="queryRef" :inline="true" v-show="showSearch">
      <el-form-item label="表名称" prop="tableName">
        <el-input
          v-model="quejlParams.tableName"
          placeholder="请输入表名称"
          clearable
          style="width: 200px"
      ...
      <el-form-item label="表描述" prop="tableComment">
        <el-input
          v-model="quejlParams.tableComment"
          placeholder="请输入表描述"
          clearable
          style="width: 200px"
          @keyup.enter="handleQuery"
    ...
    <el-table ref="genRef" v-loading="loading" :data="tableList" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
      ...
      <el-table-column label="序号" type="index" width="50" align="center">
        <template #default="scope">
          <span>{{(quejlParams.pageNum - 1) * quejlParams.pageSize + scope.$index + 1}}</span>
        </template>
      </el-table-column>
...
import type { GenTable, GenQuejlParams } from '@/types/api/tool/gen'
...
const { quejlParams, preview } = toRefs(data)

onActivated(() => {
  const time = route.query.t
  if (time != null && time != uniqueId.value) {
    uniqueId.value = time
    quejlParams.value.pageNum = Number(route.query.pageNum)
    dateRange.value = []
...
function getList() {
  loading.value = true
  listTable(proxy.addDateRange(quejlParams.value, dateRange.value)).then(response => {
...
function handleGenTable(row: GenTable) {
  ...
  if (row.genType === "1") {
    ...
    const zipName = Array.isArray(tbNames) ? "jonlink.zip" : tbNames + ".zip"
...
function handleSortChange(column: any) {
  quejlParams.value.orderByColumn = column.prop
  quejlParams.value.isAsc = column.order
  getList()
}
...
function handleEditTable(row?: GenTable) {
  ...
  const params = { pageNum: quejlParams.value.pageNum }
