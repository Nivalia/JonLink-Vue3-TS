...
import type { SysJobLog, JobLogQuejlParams } from '@/types/api/monitor/jobLog'
...
function getList() {
  loading.value = true
  listJobLog(proxy.addDateRange(quejlParams.value, dateRange.value)).then(response => {
...
(() => {
  ...
  if (jobId !== undefined && jobId != 0) {
    getJob(jobId).then(response => {
      quejlParams.value.jobName = response.data!.jobName
      quejlParams.value.jobGroup = response.data!.jobGroup
      getList()
    })
  } else {
    getList()
