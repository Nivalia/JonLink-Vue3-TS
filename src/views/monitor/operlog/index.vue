   ...
   <div class="app-container">
      <el-form :model="quejlParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
         ...
         <el-form-item label="类型" prop="businessType">
            <el-select
               v-model="quejlParams.businessType"
               placeholder="操作类型"
               clearable
               style="width: 240px"
            >
...
import type { SysOperLog, OperlogQuejlParams } from '@/types/api/monitor/operlog'
...
const data = reactive({
  ...
  quejlParams: {
    ...
    status: undefined
  } as OperlogQuejlParams
})

const { quejlParams, form } = toRefs(data)

/** 查询登录日志 */
function getList() {
  loading.value = true
  list(proxy.addDateRange(quejlParams.value, dateRange.value)).then(response => {
