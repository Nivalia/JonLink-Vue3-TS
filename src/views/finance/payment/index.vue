<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5"><el-button type="primary" plain icon="Plus" @click="openAdd" v-hasPermi="['finance:payment:add']">新增付款</el-button></el-col>
      <el-col :span="1.5"><el-button type="success" plain icon="Check" :disabled="single" @click="confirm" v-hasPermi="['finance:payment:confirm']">确认付款</el-button></el-col>
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleChange" stripe border>
      ...
      <el-table-column label="付款账户" prop="bankAccountName" min-width="160" show-overflow-tooltip>
        <template #default="{row}"><span>{{ row.bankAccountName || '' }}</span></template>
      </el-table-column>
      <el-table-column label="金额" prop="amount" width="140" align="right">
        <template #default="{row}"><b class="money out">{{ fmt(row.amount) }}</b></template>
      </el-table-column>
      ...
      <el-table-column label="日期" prop="billDate" width="120" align="center">
        <template #default="{row}"><span>{{ row.billDate ? row.billDate.substring(0,10) : '-' }}</span></template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" title="新增付款单" width="560px" append-to-body destroy-on-close>
      <el-form label-width="100px" label-position="right">
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="付款账户">
              <el-select v-model="form.bankAccountName" filterable placeholder="选择付款账户" style="width:100%" @change="matchAccountId">
        ...
        <el-form-item label="备注"><el-input v-model="form.remark" type="textarea" :rows="2" placeholder="选填" maxlength="200" show-word-limit /></el-form-item>
      </el-form>
      <template #footer>
...
import { paymentList, addPayment, confirmPayment, activeAccounts } from '@/api/finance/fund'
...
async function getList() {
  loading.value = true
  try { const r: any = await paymentList(queryParams); dataList.value = r.rows || []; total.value = r.total || 0 }
...
async function submit() {
  ...
  await addPayment(form); ElMessage.success('新增成功,未确认状态'); dialogVisible.value = false; getList()
}
async function confirm() {
  await confirmPayment(ids.value[0]); ElMessage.success('付款已确认'); getList()
}
