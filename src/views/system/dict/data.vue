   ...
   <div class="app-container">
      <el-form :model="quejlParams" ref="queryRef" :inline="true" v-show="showSearch">
         <el-form-item label="字典名称" prop="dictType">
            <el-select v-model="quejlParams.dictType" style="width: 200px">
               <el-option
         ...
         <el-form-item label="状态" prop="status">
            <el-select v-model="quejlParams.status" placeholder="数据状态" clearable style="width: 200px">
               <el-option
...
import type { SysDictData, SysDictType, DictDataQuejlParams } from '@/types/api/system/dict'
...
function getTypes(dictId: number) {
  getType(dictId).then(response => {
    quejlParams.value.dictType = response.data!.dictType
...
function getList() {
  loading.value = true
  listData(quejlParams.value).then(response => {
...
function resetQuery() {
  ...
  quejlParams.value.dictType = defaultDictType.value
  handleQuery()
}
...
function handleAdd() {
  ...
  open.value = true
  title.value = "添加字典数据"
  form.value.dictType = quejlParams.value.dictType
}
    ...
    if (valid) {
      if (form.value.dictCode != undefined) {
        updateData(form.value).then(() => {
          useDictStore().removeDict(quejlParams.value.dictType!)
