<template>
  <el-dialog v-model="visible" :title="`「${noticeTitle}」已读用户`" width="760px" top="6vh" append-to-body @close="handleClose">
    <el-form ref="queryRef" :model="quejlParams" size="small" :inline="true" style="margin-bottom: 4px;">
      <el-form-item prop="searchValue">
        <el-input
          v-model="quejlParams.searchValue"
...
import type { NoticeReadUser, NoticeReadUserQuejlParams, SysNotice } from "@/types/api/system/notice"
...
const quejlParams = reactive<NoticeReadUserQuejlParams>({
  pageNum: 1,
  pageSize: 10,
  noticeId: undefined,
...
function open(row: SysNotice) {
  quejlParams.noticeId = row.noticeId
  noticeTitle.value = row.noticeTitle ?? ""
  quejlParams.searchValue = undefined
...
function getList() {
  loading.value = true
  listNoticeReadUsers(quejlParams).then(res => {
