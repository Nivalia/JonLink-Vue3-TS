...
watch(
  ...
  (name) => {
    ...
    setFeatureHighlight(name);
  },
);

// 外部联动：drillName 变化（左侧区县列表点击）→ 下钻到对应区县。
// 地图自身点击已下钻（emit select 同步更新 selected/drillName），
// 通过 drillStack 顶部 regionName 去重避免重复下钻。
watch(
  () => props.drillName,
  async (name) => {
    ...
    if (currentState.scope === 'district') {
      ...
      if (top && top.state.regionName === name) return;
      await drillBack();
    }
...
const cameraViewConfig: CameraViewConfig = {
  ...
  byScope: {
    // 石家庄单市：相机拉近并对准地图组中心(-16,-42,-22)，
    // 让 22 区县几何体填满 canvas，减少左右/上下黑边。
    city: {
      fov: 31,
      position: [64, -644, 417],
      target: [-16, -42, -22],
    },
    // 下钻到区县：几何体经 mapScale 自适应后同样占满逻辑画布，
    // 相机沿用石家庄适配视角，保证区县地图也填满无黑边。
    district: {
      fov: 31,
      position: [64, -644, 417],
      target: [-16, -42, -22],
...
function getScopeTransform() {
  ...
  // 石家庄单市（city scope）：scale 保持 1.0，几何体占满逻辑画布(804x474)，
  // 避免 0.768 缩放导致 canvas 左右/上下大量黑边。
  return {
    position: new THREE.Vector3(-16, -42, -22),
    scale: currentState.scope === 'country' ? 0.7 : 1.0,
  };
}
...
function getEffectSourceLabelName(labels = currentLabels) {
  ...
  if (currentState.scope === 'city') {
    ...
    // 石家庄：辐射源固定从主城区（长安区/桥西区/新华区/裕华区）向外扩散，
    // 避免 hash 伪随机命中远郊县（如深泽县）造成视觉歧义。
    return labels.find((label) => /长安区|桥西区|新华区|裕华区/.test(label.name))?.name
      ?? labels[hashString(currentState.code || currentState.regionName) % labels.length]?.name;
  }
  return undefined;
}
...
function setup() {
  ...
  // 石家庄 22 区县支持下钻（DataV 区县级 GeoJSON）：创建 drill 控制条（返回上级）
  createDrillControl();
...
onMounted(async () => {
  setup();
  // initialMapState.geoData 是空占位, 首次进入异步加载市级数据并重建
  if (!geoData.features || geoData.features.length === 0) {
    const cityGeo = await loadMapLevel('city', '130100');
    if (cityGeo) {
      geoData = cityGeo as typeof geoData;
      currentState = { ...currentState, geoData: cityGeo as typeof geoData };
      await rebuildMapForCurrentState();
    }
  }
